      ...
      <div class="mb-2">
        <p>
          <span class="mr-4" style="color: #ffb0b0">
            Tổng tiền rút: <b>{{ formatPrice(dtUSD, 2) }} USDT</b>
          </span>
          <span class="mr-4" style="color: #00ffad">
            Tổng tiền BNB rút:
            <b>{{ formatPrice(dtBNB, 10) }} BNB </b>
          </span>
          <span class="mr-4" style="color: #ffa500">
            Phí: <b> {{ formatPrice(feeBNB, 10) }} BNB</b>
          </span>
        </p>
      </div>
      <vs-table
  ...
  methods: {
    ...
    async deleteMultiple() {
      ...
      for (var i = this.selectedUser.length - 1; i >= 0; i--) {
        ...
        AuthenticationService.deleteTrashByID(id);
    ...
    deleteWithdra(id, index) {
      let token = localStorage.getItem("token");
      this.$store.dispatch("setToken", token);
      AuthenticationService.deleteTrashByID(id).then((resp) => {
