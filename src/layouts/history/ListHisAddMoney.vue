      ...
      <div class="mb-2">
        <p>
          <span class="mr-4" style="color: #00ffad"
            >Tổng tiền đã thêm USDT: <b>{{ formatPrice(tUSD, 2) }}</b></span
          >
          <span class="mr-4" style="color: #00ffad"
            >Tổng tiền đã thêm BTC: <b>{{ formatPrice(tBTC, 2) }}</b></span
          >
          <span class="mr-4" style="color: #00ffad"
            >Tổng tiền đã thêm ETH: <b>{{ formatPrice(tETH, 2) }}</b></span
          >
          <span class="mr-4" style="color: #00ffad"
            >Tổng tiền đã thêm PAYPAL:
            <b>{{ formatPrice(tPAYPAL, 2) }}</b></span
          >
          <span class="mr-4" style="color: #00ffad"
            >Tổng tiền đã thêm VN: <b>{{ formatPrice(tVN, 2) }} </b></span
          >
        </p>
      </div>
      <vs-table
        ...
        <template slot-scope="{ data }">
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="de-amount">
                  USDT: {{ formatPrice(tr.price_USDT, 2) }}<br />
                </p>
                <p class="de-amount">
                  ETH: {{ formatPrice(tr.price_ETH, 4) }}<br />
                </p>
                <p class="de-amount">
                  BTC: {{ formatPrice(tr.price_BTC, 6) }}<br />
                </p>
                <p class="de-amount">
                  Paypal: {{ formatPrice(tr.price_PAYPAL, 2) }}<br />
                </p>
                <p class="de-amount">
                  VN: {{ formatPrice(tr.price_VN, 0) }}<br />
                </p>
              </vs-td>
...
export default {
  ...
  methods: {
    ...
    reloadList() {
      ...
      AuthenticationService.getAddMoneyListHistory().then((resp) => {
    ...
    getTotalAddMoney() {
      AuthenticationService.getTotalAddMoney().then((resp) => {
        let g = resp.data;
        if (g.code == 10000) {
