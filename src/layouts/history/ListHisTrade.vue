  ...
  <div id="list-trade-member" class="data-list-container">
    ...
    <div id="loading-corners" class="vs-con-loading__container">
      <div class="mb-2">
        ...
        <p>
          <span class="mr-4" style="color: #ffb0b0"
            >Tổng tiền rút: <b>{{ formatPrice(dtUSDR, 2) }} USDT</b></span
          >
          <!-- <span class="mr-4" style="color: #00ffad;">Tổng tiền BNB rút: <b>{{ formatPrice(dtBNBR, 10) }} BNB</b></span>  -->
          <span class="mr-4" style="color: #ffa500"
            >Phí: <b>{{ formatPrice(feeBNBR, 10) }} BNB</b></span
          >
        </p>
        <p>
          <span class="mr-4" style="color: #ffb0b0"
            >Tổng: <b>{{ formatPrice(dtUSD - dtUSDR, 2) }} USDT</b></span
          >
          <!-- <span class="mr-4" style="color: #00ffad;">Tổng BNB: <b>{{ formatPrice(dtBNB - dtBNBR, 10) }} BNB</b></span>  -->
          <span class="mr-4" style="color: #ffa500"
            >Tổng Phí: <b>{{ formatPrice(feeBNB - feeBNBR, 10) }} BNB</b></span
          >
        </p>
      </div>
      <vs-table
        ...
        <template slot-scope="{ data }">
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              <vs-td>
                <p class="trade-account font-medium truncate">
                  Từ: {{ tr.from_u }}<br />Đến: {{ tr.to_u }}<br /><br />Mô tả:
                  {{ tr.type }}
                </p>
              </vs-td>
              ...
              <vs-td class="whitespace-no-wrap text-center"> </vs-td>
              <vs-td class="whitespace-no-wrap text-center">
                ...
                >
                  <vs-button
                    ...
                    icon-pack="feather"
                    icon="icon-trash"
                    @click.stop="deleteTrade(tr.id, indextr, 1)"
                  ></vs-button>
                </vx-tooltip>
                <vx-tooltip
                  v-else
                ...
                >
                  <vs-button
                    ...
                    @click.stop="deleteTrade(tr.id, indextr, 0)"
                  ></vs-button>
                </vx-tooltip>
              </vs-td>
            </vs-tr>
...
export default {
  ...
  methods: {
    ...
    getTrashTrade() {
      ...
      AuthenticationService.gethistoryAllTrash().then((resp) => {
        if (!resp.data.success) {
          localStorage.removeItem("token");
          this.$router.push("/pages/login").catch(() => {});
        } else {
    ...
    reloadList() {
      ...
      AuthenticationService.getTradeListHistory().then((resp) => {
